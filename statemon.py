...
All of the variables are namespaced by module name. So, if the above
...
class State(object):
    ...
    def __init__(self):
        ...
        # We're going to assume that statemon is in the toplevel directory,
        # so we don't need to worry about trying to find it--which we do in
        # the 'real' version of statemon.
        self._NEON_ROOT = os.path.abspath(os.path.dirname(__file__))
    ...
    def define_and_decrement(self, name, diff=1, typ=int, safe=True,
                             stack_depth=1):
        """The same as define_and_increment, and in fact just wraps it."""
        self.define_and_increment(name, -diff, typ, safe, stack_depth)

    dni = define_and_increment
    dnd = define_and_decrement
